...
### d. Luồng xử lý chi tiết RAG Agent

Chi tiết quy trình Agentic RAG: từ lúc `RagAgent` nhận yêu cầu, quyết định chiến lược tìm kiếm, tương tác với Qdrant và tổng hợp câu trả lời.

```mermaid
flowchart TD
    Start([Input Context]) --> RagAgent{RagAgent Decision}

    %% Decision branches
    RagAgent -->|Need Better Query| QueryGen[Query Creating Agent]
    RagAgent -->|Need Classification| Topic[Topic Classify Agent]
    RagAgent -->|Context Sufficient| Compose[Compose Answer Node]

    %% Retrieval paths
    QueryGen -->|New Query| Retrieve[Retrieve From KB]
    Topic -->|DEMUC Info| Retrieve

    %% Retrieval Action
    Retrieve --> Qdrant[(Qdrant Vector DB)]
    Qdrant -->|Context Docs| Check{Sufficient Info?}

    %% Loop check
    Check -->|No - Try Again| RagAgent
    Check -->|Yes - Ready| Compose

    %% Final output
    Compose --> Output([Final Answer])

    %% Styling
    style RagAgent fill:#f9f,stroke:#333,stroke-width:2px
    style Qdrant fill:#bbf,stroke:#333,stroke-width:2px,shape:cylinder
```
