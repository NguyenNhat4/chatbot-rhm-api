...
Mô tả tóm tắt quy trình truy xuất thông tin (RAG) khi hệ thống quyết định cần tra cứu kiến thức.

```mermaid
sequenceDiagram
    autonumber
    participant Flow as MedFlow
    participant RagAgent as RagAgent
    participant Retrieval as Retrieval Node
    participant Qdrant as Qdrant Vector DB
    participant Compose as ComposeAnswer

    Flow->>RagAgent: Kích hoạt (User Query)
    activate RagAgent

    RagAgent->>RagAgent: Phân tích chiến lược (Dùng Query cũ / Tạo mới / Phân loại)

    RagAgent->>Retrieval: Yêu cầu tìm kiếm
    activate Retrieval

    Retrieval->>Qdrant: Hybrid Search (Dense + Sparse + ColBERT)
    Qdrant-->>Retrieval: Danh sách tài liệu (Context)

    Retrieval-->>Flow: Trả về Context Documents
    deactivate Retrieval
    deactivate RagAgent

    Flow->>Compose: Tổng hợp câu trả lời
    activate Compose
    Compose->>Compose: Call LLM (Context + Memory + Query)
    Compose-->>Flow: Câu trả lời cuối cùng
    deactivate Compose
```
